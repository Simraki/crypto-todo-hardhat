...
import { prepareERC20Tokens, prepareSigners, prepareTicTacToeAndMultiSigWallet } from "./utils/prepare"
import { duration, increase } from "./utils/time"

use(waffle.solidity)

describe("TicTacToe contract", function () {
    const plainDec6 = ethers.utils.parseUnits("1", 6)
    ...
    describe("Creating and starting via ETH", function () {
        ...
        testData.forEach((datum) => {
            ...
            describe(name, function () {
                beforeEach(async function () {
                    await prepareTicTacToeAndMultiSigWallet(this, this.owner, fee, isAbsFee)
                })
                ...
                it("should create empty game", async function () {
                    ...
                    const balanceMSG = await ethers.provider.getBalance(this.MSG.address)
                    expect(balanceMSG).to.equal(0)
                })
                ...
                it("should create own game", async function () {
                    ...
                    // Check MultiSigWallet
                    const balanceMSG = await ethers.provider.getBalance(this.MSG.address)
                    expect(balanceMSG).to.equal(feePerStake)
                })
                ...
                it("should join the game", async function () {
                    ...
                    // Check MultiSigWallet
                    const balanceMSG = await ethers.provider.getBalance(this.MSG.address)
                    expect(balanceMSG).to.equal(feePerStake.mul(2))
                })
        ...
        testData.forEach((datum) => {
            ...
            describe(name, function () {
                ...
                it("should create empty game", async function () {
                    ...
                    // Check MultiSigWallet
                    const balanceMSG = await this.token1.balanceOf(this.MSG.address)
                    expect(balanceMSG).to.equal(0)
                })
    ...
    describe("Moving via ETH", function () {
        const stake = ethers.utils.parseEther("2")
        const tokenAddress = ethers.constants.AddressZero
        ...
        beforeEach(async function () {
            await prepareTicTacToeAndMultiSigWallet(this, this.owner, fee, false)
